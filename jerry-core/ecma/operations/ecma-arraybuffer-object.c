                                ...
                                bool shared)
{
  ...
  /* 2. */
  if (shared != ecma_object_class_is (object_p, ECMA_OBJECT_CLASS_SHARED_ARRAY_BUFFER))
  {
    return ecma_raise_type_error (ECMA_ERR_MSG ("Argument 'this' is not an SharedArrayBuffer object"));
  }
  else if (!shared != ecma_object_class_is (object_p, ECMA_OBJECT_CLASS_ARRAY_BUFFER))
  {
